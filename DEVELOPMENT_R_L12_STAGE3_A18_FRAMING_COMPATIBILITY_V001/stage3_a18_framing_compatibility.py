...
STAGE3_MODULE_NAME: Final[str] = "authenticated_stage3_entry_adapter_a18_frame"
...
SELF_FREEZE_SCHEMA: Final[str] = (
    "V012_STAGE3_STORED_PUBLICATION_FRAMING_COMPATIBILITY_SOURCE_FREEZE_V001"
)
...
def _load_stage3() -> types.ModuleType:
    ...
    existing = sys.modules.get(STAGE3_MODULE_NAME)
    if existing is not None:
        if (
            getattr(existing, "__file__", None) != str(STAGE3_ADAPTER)
            or getattr(existing, "__authenticated_sha256__", None)
            != PINNED_STAGE3_PACKET[STAGE3_ADAPTER.name]
        ):
            raise FramingRefusal("authenticated Stage3 module alias mismatch")
        return existing
    raw = _read_pinned_stage3_member(STAGE3_ADAPTER.name)
...
def _parse_exact_publication(
    ...
    if _publication_json_bytes(value) != raw:
        raise FramingRefusal(
            f"{contract.name} does not use exact Stage2 publication framing"
        )
    if (
...
@contextmanager
def _patched_pretty_inputs_open(
    contracts: tuple[_PrettyInputContract, ...] = PRODUCTION_PRETTY_INPUTS,
) -> Iterator[None]:
    ...
    if not isinstance(original_descriptor, classmethod):
        raise FramingRefusal("frozen StableInput.open descriptor changed")
    ...
    def compatible_open(cls, label: str, path: Path, *, json_record: bool):
        ...
        try:
            ...
            if retained.digest != contract.sha256:
                raise FramingRefusal(f"{contract.name} exact publication hash mismatch")
...
def plan() -> dict[str, object]:
    ...
    return {
        "schema": "V012_STAGE3_STORED_PUBLICATION_FRAMING_COMPATIBILITY_PLAN_V001",
        "classification": (
