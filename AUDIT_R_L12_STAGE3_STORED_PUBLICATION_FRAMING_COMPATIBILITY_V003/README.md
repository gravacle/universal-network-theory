# Mutable Stage-3 exact pretty-input compatibility V003 candidate
...
V003 retains the independently reviewed V002 behavior, changes the registered
A18 SHA-256 from the retired pre-replay cross-gate to the passing data-lineage
replay cross-gate `defab8ec...`, and admits that replay's exact compact frame.
The target manifest remains exact Stage-2 publication framing. It refuses
preexisting Stage-3 module aliases as authority: it
always executes retained authenticated adapter bytes into a fresh module and
checks the loaded module identity.  Its production patch context accepts no
contract arguments.  Test fixtures exercise the internal opener separately.
Each installed opener holds an active capability that is revoked before the
original classmethod is restored, so a leaked bound callable refuses after
scope exit; nested and reentrant scopes also refuse.
