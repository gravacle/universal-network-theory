#!/usr/bin/env python3
"""Matrix-free L8 connected-cycle autonomous BS09 accumulation."""
...
psi0[(words & even_mask) == 0] = 1.0 / math.sqrt(1 << (SITES // 2))
...
l8_over_l4_per_retained = (
    (COMPONENTS * throughput_component / 128.0)
    / (l4_active["absolute_oriented_throughput_global"] / 16.0)
)
l8_over_l6_total = (
    COMPONENTS * throughput_component
    / l6["absolute_oriented_throughput_global"]
)
l8_over_l6_per_retained = (
    (COMPONENTS * throughput_component / 128.0)
    / (l6["absolute_oriented_throughput_global"] / 54.0)
)
...
checks = [
    (SITES == 16 and DIMENSION == 65536, "component census"),
    (len(edges) == 24 and np.sum(connector_mask) == 8, "owner-once edge census"),
    (all(len(neighbors[vertex]) == 3 for vertex in range(SITES)), "degree-three support"),
    (len(visited) == SITES, "component connected"),
    (COMPONENTS * SITES == 512, "global L8 site census"),
    (COMPONENTS * len(edges) == 768, "global owner-once edge census"),
    (int(np.sum(np.abs(currents[internal_mask]) > 1.0e-10)) == 16, "all internal currents active"),
    (int(np.sum(np.abs(currents[connector_mask]) > 1.0e-10)) == 8, "all connector currents active"),
    (connector_throughput_component > 1.0e-6, "nonzero connector throughput"),
    (abs(COMPONENTS * np.sum(q1) - 128.0) < 2.0e-8, "global retained total"),
    (state_refinement_linf < 2.0e-10, "state refinement"),
    (occupation_refinement_linf < 2.0e-10, "occupation refinement"),
    (current_refinement_linf < 2.0e-10, "current refinement"),
    (float(np.sum(np.abs(residuals))) < 4.0e-10, "fine ledger L1"),
    (float(np.max(np.abs(residuals))) < 4.0e-11, "fine ledger Linf"),
    (float(np.sum(np.abs(residuals))) <= float(np.sum(np.abs(residuals_coarse))) + 2.0e-11, "ledger refinement nonworsening"),
    (abs(np.vdot(psi, psi).real - 1.0) < 2.0e-10, "norm"),
    (abs(np.vdot(psi, hpsi).real - np.vdot(psi0, hpsi0).real) < 2.0e-9, "energy"),
    (float(np.max(np.abs(number_law - number_law0))) < 2.0e-10, "number law"),
    (l8_over_l4_total > 0.0 and l8_over_l4_per_retained > 0.0, "L8/L4 comparator"),
    (l8_over_l6_total > 0.0 and l8_over_l6_per_retained > 0.0, "L8/L6 comparator"),
]
failures = [label for ok, label in checks if not ok]

out = {
    "schema": "R_AUTONOMOUS_CONNECTED_L8_ACCUMULATION_V001",
    ...
    "census": {
        "L": 8,
        "sites_global": 512,
        "sites_per_F3_layer": 256,
        "possible_F3_links": 65536,
        "components": COMPONENTS,
        "sites_per_component": SITES,
        "internal_edges_per_component": 16,
        "connector_edges_per_component": 8,
        "selected_edges_global_owner_once": COMPONENTS * len(edges),
        "prepared_source_lineages": 256,
        "expected_retained_global": 128,
    },
    "parameters": {
        "kappa": KAPPA,
    ...
    "L8_over_L4": {
        "throughput_total": l8_over_l4_total,
        "throughput_per_retained_record": l8_over_l4_per_retained,
    },
    "L8_over_L6": {
        "throughput_total": l8_over_l6_total,
        "throughput_per_retained_record": l8_over_l6_per_retained,
    },
    "owner_once_action": "H_EQUALS_MINUS_T_SUM_OVER_768_UNIQUE_EDGES_T_E__32_IDENTICAL_CONNECTED_COMPONENTS",
...
print(f"PASS__R_AUTONOMOUS_CONNECTED_L8_ACCUMULATION__{len(checks)}/{len(checks)}")
