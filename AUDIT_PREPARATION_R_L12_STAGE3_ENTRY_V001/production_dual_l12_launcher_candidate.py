...
def publication_json_bytes(record: object) -> bytes:
    """Exact sorted indent-2 encoding used by target predecessor publishers."""
    try:
        return (json.dumps(
        ...
        raise LaunchRefusal("record is not finite publication JSON") from error
...
class StableInput:
    ...
    @classmethod
    def open(
        cls, label: str, path: Path, *, json_record: bool,
        json_encoding: str = "compact",
    ) -> "StableInput":
        if json_encoding not in {"compact", "publication"}:
            raise LaunchRefusal(f"{label} JSON encoding policy is invalid")
        ...
        try:
            ...
            if json_record:
                ...
                expected_raw = (
                    ...
                    if json_encoding == "compact"
                    else publication_json_bytes(value)
                )
                if type(value) is not dict or expected_raw != raw:
...
def execute(execution_guard: object | None = None) -> str:
    ...
    try:
        publication_labels = {"L10 cross gate", "target cache manifest"}
        for label, path, is_json in specifications:
            retained.append(StableInput.open(
                label, path, json_record=is_json,
                json_encoding=(
                    "publication" if label in publication_labels else "compact"
                ),
            ))
